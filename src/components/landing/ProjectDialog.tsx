        ...
        <div className="grid gap-6 py-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-background-alt p-4 rounded-lg border border-border/50">
            <div>
              ...
              <p className="font-body font-medium text-foreground">{t(`projects-${project.id}-role` as any) || project.role}</p>
            </div>
            <div>
              <span className="text-xs text-muted-foreground font-code uppercase tracking-widest">{t("landing-portfolio-dialog-status")}</span>
              <p className="font-body font-medium text-foreground">{t(`projects-${project.id}-status` as any) || project.status}</p>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="font-body text-muted-foreground leading-relaxed">{t(`projects-${project.id}-challenge` as any) || project.challenge}</p>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {project.solution.map((_, idx) => (
                <li key={idx} className="flex gap-3 text-muted-foreground">
                  <CheckCircle2 className="w-5 h-5 text-primary shrink-0 mt-0.5" />
                  <span className="font-body leading-relaxed">{t(`projects-${project.id}-solution-${idx}` as any) || project.solution[idx]}</span>
                </li>
              ))}
            </ul>
          </div>
          
          {(project.seniorInsight || t(`projects-${project.id}-insight` as any)) && (
            <div className="bg-primary/5 border border-primary/20 rounded-lg p-5 mt-2 relative overflow-hidden">
              ...
              <p className="font-body text-foreground italic leading-relaxed">"{t(`projects-${project.id}-insight` as any) || project.seniorInsight}"</p>
            </div>
          )}
